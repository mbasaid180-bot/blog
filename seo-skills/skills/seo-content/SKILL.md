...
After content analysis, suggest relevant follow-up actions:
- If schema issues found → `/seo-schema <url>` to fix structured data
- If GEO score low → `/seo-geo <url>` to optimize for AI citations
- If images need work → `/seo-images <url>` to optimize images
- If content is good → `/seo-wordpress-publish` to publish
- If content needs rewrite → `/seo-write-article <keyword>` to regenerate
