...
| `/seo-cluster plan --from strategy` | Import from existing `/seo-wp-strategy` output |
...
1. Look for the most recent `/seo-wp-strategy` output in the current directory (search for
   files matching `*strategy*`, `*content-strategy*`, `*editorial*`)
...
If no strategy file is found, prompt the user: "No existing strategy found in the
current directory. Run `/seo-wp-strategy` first, or provide a seed keyword for fresh clustering."
...
4. Inform user: "Briefs saved to `cluster-briefs/`. Use `/seo-write-article` to write each article."
...
| Skill | Relationship |
|-------|-------------|
| `seo-wp-strategy` | Import source: strategy import reads wp-strategy output |
...
| `seo-write-article` | Write articles from cluster plan |
| `seo-outline-article` | Generate outlines for each cluster article |

After cluster planning or execution completes, suggest:
"Start writing? Use `/seo-outline-article <keyword>` then `/seo-write-article <keyword>`"
...
| "No strategy file found" | `--from strategy` but no plan exists | Prompt user to run `/seo-wp-strategy` first |
